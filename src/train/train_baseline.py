...
def save_checkpoint(epoch, model, optimizer, scheduler, filename='weights/baseline_latest.pth'):
    torch.save({
        'epoch': epoch,
...
def train():
    ...
    checkpoint_path = 'weights/baseline_latest.pth'
    start_epoch = 0
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # 每10个epoch保存一个独立模型文件
        if (epoch + 1) % 10 == 0:
            torch.save(model.state_dict(), f'weights/baseline_epoch{epoch+1}.pth')
            print(f"独立模型已保存: weights/baseline_epoch{epoch+1}.pth")

        scheduler.step()

    print("训练完成！最终模型已保存至 weights/baseline_final.pth")
    torch.save(model.state_dict(), 'weights/baseline_final.pth')
